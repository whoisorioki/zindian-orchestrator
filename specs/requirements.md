...
### REQ-08: Environment pinning

The repository **MUST** include a `requirements.in` file listing the top-level,
intended runtime and research dependencies (unpinned). A pinned
`requirements.txt` generated by `pip-compile` (from `pip-tools`) **MUST** be
committed alongside `requirements.in` and represents the reviewable, exact
environment used for reproducing results. `skill_22` will validate that
`requirements.txt` exists and that `requirements.in` was used to generate it.
