...
def test_composite_fold_variance_multi_target():
    ...
    assert abs(metric_analysis["composite_fold_score_variance"] - expected_variance) < 1e-6
...
def test_single_target_backward_compatibility():
    ...
    assert "composite_fold_score_variance" not in metric_analysis or \
           metric_analysis.get("composite_fold_score_variance") is None
