...
def make_dummy_data(n=100, features=5, seed=0):
    ...
    df["target"] = y
    test_df = pd.DataFrame(rng.randn(int(n/5), features), columns=[f"f{i}" for i in range(features)])
    return df, test_df
...
def test_train_lightgbm_cv_deterministic_with_seed():
    ...
    res1 = train_lightgbm_cv(train, test, feature_cols, "target", n_splits=4, random_seed=42)
    res2 = train_lightgbm_cv(train, test, feature_cols, "target", n_splits=4, random_seed=42)
...
def test_train_lightgbm_cv_varies_with_different_seed():
    ...
    res1 = train_lightgbm_cv(train, test, feature_cols, "target", n_splits=4, random_seed=1)
    res2 = train_lightgbm_cv(train, test, feature_cols, "target", n_splits=4, random_seed=7)
