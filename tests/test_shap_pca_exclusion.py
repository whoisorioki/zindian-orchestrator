...
class TestPcaExclusionSingleTarget:
    ...
    def _run_with_all_mocked(self, tmp_path, frame, monkeypatch):
        ...
        monkeypatch.setattr(s10, "resolve_competition_paths",
                            lambda require_competition=False, **kw: _fake_paths(tmp_path, store))
        monkeypatch.setattr(s10, "ChallengeConfig",
        ...
        monkeypatch.setattr(s10, "_build_pruned_feature_set", lambda *a, **kw: {
            "correlated_pairs": [],
            "drop_features": [],
            "pruned_features": ["feature_a", "feature_b"],
        })
        monkeypatch.setattr(s10, "train_lightgbm_cv", lambda *a, **kw: _stub_cv_result())
    ...
    def test_pca_cols_not_in_compute_shap_audit(self, tmp_path, monkeypatch):
        ...
        for cols in received:
            ...
            assert not pca_in_audit, f"PCA columns leaked into SHAP audit: {pca_in_audit}"
...
class TestPcaExcludedGateIntegration:
    ...
    def test_pca_excluded_skip_reason_satisfies_shap_condition(self, tmp_path, monkeypatch):
        """
        ...
        monkeypatch.setattr(s11, "resolve_competition_paths",
                            lambda **kw: _FakePaths())
        monkeypatch.setattr(s11, "ChallengeConfig",
                            MagicMock(load=MagicMock(return_value=fake_config)))
