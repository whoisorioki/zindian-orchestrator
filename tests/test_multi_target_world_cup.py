...
def test_world_cup_intake_a11_compliance():
    ...
    try:
        # Mock config
        config = {"metric": "composite", "task_type": "multi_target"}
        ...
        total_goals = next(
            t for t in target_config["targets"] if t["name"] == "total_goals"
        )
        target_col = next(t for t in target_config["targets"] if t["name"] == "Target")

        # Validate total_goals (regression)
        assert (
            total_goals["task_type"] == "regression"
        ), "total_goals must be regression"
        assert total_goals["metric"] in (
            "rmse",
            "root_mean_squared_error",
        ), "total_goals must use RMSE"
...
def test_world_cup_intake_a12_compliance():
    ...
    try:
        ...
        ), "A12 policy detection must return target_config"

        # A12 Compliance: Policy must be present for mixed-task
        assert (
            "pseudo_label_recombination_policy" in target_config
...
def test_world_cup_anchor_composite_aggregation():
    ...
    ), f"Composite score incorrect: {composite} != {expected}"
...
def test_world_cup_leak_gate_single_target_blocks_all():
    ...
    leaked_features_Target: list[str] = []
    ...
    # Test clean scenario
    leaked_features_total_goals_clean: list[str] = []
    has_leaks_clean = (
        len(leaked_features_total_goals_clean) > 0 or len(leaked_features_Target) > 0
    )
...
def test_world_cup_a12_freeze_policy():
    ...
    assert np.array_equal(
        composite_oof["Target"], augmented_oof_Target
    ), "Classification OOF must be augmented"
    assert np.array_equal(
        composite_oof["total_goals"], original_oof_total_goals
...
def test_world_cup_a12_block_policy():
    ...
    regression_targets_empty: list[dict] = []
    can_proceed_clean = len(regression_targets_empty) == 0
    assert (
        can_proceed_clean
    ), "A12 block policy must proceed when all targets are classification"
...
def test_single_target_backward_compatibility():
    ...
    try:
        ...
        ), "Single-target competitions must not generate target_config"
