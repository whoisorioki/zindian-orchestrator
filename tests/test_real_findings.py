...
def test_s11_no_root_writes(tmp_path, monkeypatch):
    ...
    # Ensure reports and data_processed directories exist
    mock_paths.reports_dir.mkdir(parents=True, exist_ok=True)
    mock_paths.data_processed_dir.mkdir(parents=True, exist_ok=True)
    ...
    assert not (mock_paths.reports_dir / "literature_cache.json").exists()
    assert not (mock_paths.reports_dir / "domain_hypotheses.json").exists()
    assert (mock_paths.reports_dir / "diagnostics" / "literature_cache.json").exists()
    assert (mock_paths.reports_dir / "diagnostics" / "domain_hypotheses.json").exists()
...
def test_preflight_mt_oof_completeness(tmp_path, monkeypatch):
    import pytest
    from scripts.preflight_enforce import verify_section_1_assumptions, PreflightError

    monkeypatch.setenv("COMPETITION_SLUG", "test-comp")
    ...
    state_pass = {
        "branch_anchor-baseline_target_1_oof": {"cv_strategy_id": "stratified"},
        "branch_anchor-baseline_target_2_oof": {"cv_strategy_id": "stratified"},
    }

    verify_section_1_assumptions(
        comp_path=comp_path,
        cfg=cfg,
        state=state_pass,
    ...
    with pytest.raises(PreflightError, match="incomplete OOF records"):
        verify_section_1_assumptions(
            comp_path=comp_path,
            cfg=cfg,
            state=state_fail,
...
def test_session_log_deduplication(tmp_path, monkeypatch):
    import json
    ...
    state_data["dag_phase"] = "phase_1_integrity_intake"
    ...
    MockPaths.root = tmp_path
    MockPaths.reports_dir = reports_dir
    ...
    class MockDatetimeClass:
        ...
        @classmethod
        def now(cls, tz=None):
            cls._curr_time += dt_module.timedelta(seconds=1)
    ...
    monkeypatch.setattr("zindian.skills.skill_15_reporter.ChallengeConfig", MockConfig)

    # First run: writes a new log
    res1 = run(
    ...
    # Enforce rolling window test: create 15 non-identical logs
    for i in range(15):
        # We modify dag_phase to make them distinct
        state_data["dag_phase"] = f"phase_distinct_{i}"
        state_path.write_text(json.dumps(state_data), encoding="utf-8")
        run(
            ledger_path=str(reports_dir / "experiments.db"),
            state_path=str(state_path),
