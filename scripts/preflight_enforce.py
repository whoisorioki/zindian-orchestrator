...
- challenge_config.json contains all required SoT schema fields
...
def check_config_completeness(cfg: dict) -> None:
    """Validate challenge_config.json fields per SoT schema."""
...
def verify_section_1_assumptions(
    ...
    ok(
        "A7 check: All OOF records carry a cv_strategy_id tag and are complete per branch"
    )
...
def main():
    ...
    parser.add_argument(
        "--repo-wide",
        action="store_true",
        help="Run repository-wide static analysis and environment checks only",
    )
    ...
    if args.repo_wide:
        print("Running repository-wide static analysis and environment checks...")
        failures = []
        try:
        ...
        if failures:
            print("\n[FAIL] Repository-wide preflight checks failed:")
            for f in failures:
                print(f"  - {f}")
            sys.exit(1)
        else:
            print("[PASS] Repository-wide preflight checks passed.")
            sys.exit(0)
