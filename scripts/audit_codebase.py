...
def audit_agents_md_ground_truth() -> list[dict]:
    ...
    results.append(check(
        ...
        "def resolve_active_cv_strategy_id" in Path("zindian/state.py").read_text(encoding="utf-8")
    ))
    results.append(check(
        "resolve_active_cv_strategy_id() NOT in cv.py",
        "def resolve_active_cv_strategy_id" not in Path("zindian/cv.py").read_text(encoding="utf-8")
    ))

    # Claim 2: write_oof_record in state.py, NOT cv.py
    results.append(check(
        "write_oof_record() in state.py",
        "def write_oof_record" in Path("zindian/state.py").read_text(encoding="utf-8")
    ))
    results.append(check(
        "write_oof_record() NOT in cv.py",
        "def write_oof_record" not in Path("zindian/cv.py").read_text(encoding="utf-8")
    ))

    # Claim 3: SkillStateStore class
    results.append(check(
        "SkillStateStore class at state.py line 29",
        "class SkillStateStore" in Path("zindian/state.py").read_text(encoding="utf-8")
    ))
    ...
    results.append(check(
        ...
        "_atomic_write_json" in state_src and "tempfile" in state_src and "os.replace" in state_src
    ))
    ...
    for f in skill_files:
        ...
        for other in skill_files:
            ...
            for pattern in [f"from .{other_mod} import", f"import .{other_mod}", f"from zindian.skills.{other_mod} import"]:
                if pattern in src:
...
def audit_skill_entry_points() -> list[dict]:
    ...
    for f in sorted(SKILL_DIR.glob("skill_*.py")):
        ...
        results.append(check(
            f"{f.name}: has run() entry point",
            has_run
        ))
        if has_run:
            # Check signature includes config/state
            run_def = src.split("def run(")[1].split("):")[0] if "def run(" in src else ""
            ...
            # Check returns dict
            returns_dict = "-> dict" in src.split("def run(")[1].split(":")[0] if "def run(" in src else False
            results.append(check(
...
def audit_v22_specific() -> list[dict]:
    ...
    results.append(check(
        "_lightgbm_shared.py: use_log1p flag",
        "use_log1p" in shared_src
    ))
    results.append(check(
    ...
    results.append(check(
        ...
        "2.2-Generalized-Regression" in sot and "v2.2-Generalized-Regression" in sot
    ))
    results.append(check(
...
def run_all() -> None:
    ...
    print(f"\n{'=' * 60}")
    print(f"Overall: {'ALL PASS ✅' if not failed else f'{len(failed)} DISCREPANCIES FOUND ❌'}")
    print(f"{'=' * 60}")
