...
checks = [
    ...
    ('expm1 after clip for rmsle', _check_text(src, 'np.expm1(np.clip(val_pred_flat, 0, None))')),
    ('domain bounds clipping for RMSE/MAE', _check_text(src, 'np.clip(val_pred_flat, domain_bounds[0], domain_bounds[1])')),
    ...
    ('Correct RMSLE score formula', _check_text(src, 'np.sqrt(np.mean((np.log1p(y) - np.log1p(oof_probs)) ** 2))')),
]
...
checks += [
    ('regression_metric from config', _check_text(src08, 'config.get("metric") if task_type == "regression" else None')),
    ...
    ('train_lightgbm_cv called with regression_metric', _check_text(src08, 'regression_metric=regression_metric')),
]
...
checks += [
    ('SCALE_INVARIANT_METRICS (rmsle)', _check_text(src11, 'SCALE_INVARIANT_METRICS = frozenset({"rmsle"})')),
    ('SCALE_SENSITIVE_METRICS defined', _check_text(src11, 'SCALE_SENSITIVE_METRICS = frozenset(')),
    ...
    ('RMSLE returns raw thresholds', _check_text(src11, 'return variance_gate_threshold, gate_margin, None')),
    ('variance scaling by target_std^2', _check_text(src11, 'variance_gate_threshold * (target_std**2)')),
    ('margin scaling by target_std', _check_text(src11, 'gate_margin * target_std')),
    ('degenerate target_std warning', 'Degenerate target_std (0.0) in skill_11_gate' in src11),
    ('catch-all for unknown regression metrics', _check_text(src11, 'metric not in SCALE_INVARIANT_METRICS')),
]
...
checks += [
    ...
    ('SoT Regression Target Transformation Lifecycle section', 'Regression Target Transformation Lifecycle' in so_src),
