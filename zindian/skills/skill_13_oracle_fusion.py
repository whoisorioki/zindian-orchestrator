...
def _classification_threshold_and_f1(y_true: np.ndarray, probs: np.ndarray) -> tuple[float, float]:
    best_f1 = 0.0
    best_t = 0.5
...
def _collect_verified_candidates(
    ...
    *,
    ...
    return sorted(candidates, key=lambda x: x["score"], reverse=(direction != "minimize"))
...
def _prune_collinear(candidates: list[dict[str, Any]], *, task_type: str, direction: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    while changed and len(working) > 1:
        ...
        for i in range(len(working)):
            for j in range(i + 1, len(working)):
                a = working[i]
                b = working[j]
                corr = _correlation(np.asarray(a["probs"], dtype=np.float64), np.asarray(b["probs"], dtype=np.float64), task_type)
                if corr <= 0.95:
                    continue
...
def run(dry_run: bool = False) -> dict:
    ...
    target_col = _resolve_target_col(config, train)
    ...
    use_probabilities = bool(config.get("use_probabilities", True))
    ...
    pruned_variants, dropped_pairs = _prune_collinear(all_variants, task_type=task_type, direction=direction)
    if not pruned_variants:
        return {"status": "FAILED", "reason": "All verified candidates dropped by correlation pruning"}

    selected = pruned_variants[: int(config.get("fusion_top_n", TOP_N) or TOP_N)]
    names = [v["name"] for v in selected]
    print(f"\nSelected after pruning: {names}")
    if dropped_pairs:
        print("Dropped collinear candidates:")
        for row in dropped_pairs:
            print(f"  dropped={row['dropped']} kept={row['kept']} corr={row['correlation']:.5f}")

    # Validate Human Gate 3 as a read-only gate for non-dry execution.
    if not dry_run and not bool(state.get("human_gate_3_approved", False)):
    ...
    anchor_metric_key = (
        ...
        else ("anchor_oof_auc" if metric_name.lower() in ("auc", "roc_auc", "roc_auc_score") else "anchor_oof_rmse")
    )
    if retraining_active:
        augmented_key = f"{anchor_metric_key}_augmented"
        anchor_score = float(state.get(augmented_key, state.get(anchor_metric_key) or 0.0) or 0.0)
    else:
        anchor_score = float(state.get(anchor_metric_key) or 0.0)
    ...
    if direction == "minimize":
        print(f"Delta (blend-anchor)       : {blend_score - anchor_score:+.6f} (lower is better)")
    else:
        print(f"Delta (blend-anchor)       : {blend_score - anchor_score:+.6f} (higher is better)")
    ...
    if len(submission_values) != len(sample):
        print(f"Row count mismatch: preds={len(submission_values)}, sample={len(sample)}")
    ...
    updates = {
        ...
        "last_ensemble_threshold": (None if blend_threshold is None else round(float(blend_threshold), 2)),
