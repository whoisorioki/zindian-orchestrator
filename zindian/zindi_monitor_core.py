...
def _parse_deadline(full_text: str) -> str | None:
    ...
    month_lookup.update(
        {abbr.lower(): index for index, abbr in enumerate(calendar.month_abbr) if abbr}
    )
...
def scrape_competition_page(slug: str) -> dict:
    ...
    with sync_playwright() as p:
        ...
        must_select_2 = (
            "choose 2 submissions" in full_text or "select 2 submissions" in full_text
        )
...
def fetch_competition_intel(slug: str, headers: dict, config=None) -> dict:
    ...
    external_hints = [
        f for f in datafiles if any(src in f.lower() for src in external_sources)
    ]
...
def classify_flag_text(text: str) -> str:
    ...
    if any(
        k in t
        for k in (
            "derived spatial features are",
            "derived spatial features are also banned",
            "are banned",
            "not allowed",
            "prohibited",
            "forbidden",
...
def extract_compliance_flags(
    ...
    for d in discussions:
        ...
        for c in comments:
            ...
            if any(
                x in username for x in ("zindi", "organizer", "organiser", "admin")
            ) or any(
...
def fetch_submission_intel(client: ZindiClient) -> dict:
    ...
    try:
        ...
        metric_direction = config.get("metric_direction", "maximize")
    except Exception:
    ...
    for raw in subs:
        ...
        if metric_direction == "minimize":
            if score > 0.0 and score < best_compliant:
...
def write_compliance_log(
    ...
    lines = [
        ...
        "## Submission Board",
        "",
        f"{'ID':<12} {'Date':<12} {metric_label:>12} {'Chosen':>8}  File",
        "-" * 100,
    ]
    ...
    print("  ✅ compliance_log.md updated")
...
def write_monitor_json(
    ...
    out = {
        ...
        "compliance": {
            ...
            "flags": flagged,
            "external_sources_mentioned": list(
            ...
            "flag_class_counts": {
                ...
                "clarify": sum(
                    1 for f in flagged if f.get("classification") == "clarify"
                ),
                "question": sum(
                    1 for f in flagged if f.get("classification") == "question"
                ),
            },
        },
    }
    ...
    print("  ✅ zindi_monitor.json updated")
...
def run(
    ...
    print(f"\n{'=' * 60}")
    print("SKILL 00 — Zindi Monitor")
    print(f"{'=' * 60}\n")

    paths = resolve_competition_paths()
    config_obj = ChallengeConfig.load()
    slug = config_obj.slug
    hdrs = _get_headers()

    # ── 1. Competition page ───────────────────────────────────
    print("[1/4] Fetching competition intelligence...")
    try:
        comp_intel = fetch_competition_intel(slug, hdrs, config=config_obj)
        ...
        if comp_intel.get("external_sources_on_data_page"):
            print(
                f"  External on data pg : {comp_intel['external_sources_on_data_page']}"
            )
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            metric_name = "F1"
        metric_label = f"LB {metric_name}"
        print(f"\n  {'ID':<12} {'Date':<12} {metric_label:>12} {'Chosen':>8}  File")
        print(f"  {'-' * 80}")
