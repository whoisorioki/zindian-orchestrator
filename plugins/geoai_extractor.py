...
class Extractor(FeatureExtractor):
    ...
    def extract(
        self, paths, tiff_path: Path, config, branch_name: str = "baseline"
        ...
        train.to_csv(paths.data_processed_dir / f"features_train_{branch_name}.csv", index=False)
        test.to_csv(paths.data_processed_dir / f"features_test_{branch_name}.csv", index=False)
        return train, test


# Backward compatibility: expose extract as module-level function
def extract(paths, tiff_path: Path, config, branch_name: str = "baseline") -> Tuple[pd.DataFrame, pd.DataFrame]:
