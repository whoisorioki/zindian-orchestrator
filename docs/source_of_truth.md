...
#### RMSLE vs RMSE Target Transformation and Evaluation
When the primary metric configured is `rmsle` (Root Mean Squared Logarithmic Error) for regression:
1. Target variables $y$ must be log-transformed prior to training: $y' = \ln(y + 1)$.
2. Predictions generated by the model will be in log-space ($\hat{y}'$).
3. To calculate the evaluation metric (RMSLE) on validation folds, predictions must be inverse-transformed back to original scale: $\hat{y} = e^{\hat{y}'} - 1$.
4. Domain clipping must be applied to prevent negative target predictions: $\hat{y} = \max(0, \hat{y})$.
5. The final RMSLE score is computed in original space: $\sqrt{\frac{1}{N} \sum (\ln(y + 1) - \ln(\hat{y} + 1))^2}$.
