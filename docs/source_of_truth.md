...
#### Workspace Resolution & Multi-Tenancy Concurrency Contract

`zindian.paths.resolve_competition_paths(slug=None, competition_dir=None, require_competition=False)` resolves project paths into an immutable `CompetitionPaths` instance.

```
Resolution Order:
  1) Direct competition_dir argument (supports external workspace paths outside repo_root/competitions)
  2) Direct slug argument (checked against competitions/<slug> and directly as a path)
  3) Current Working Directory (if inside competitions/<slug>/ or containing SKILL_STATE.json / challenge_config.json)
  4) ZINDIAN_COMPETITION_DIR, ZINDIAN_COMPETITION, COMPETITION_SLUG, or ZINDIAN_COMPETITION_SLUG env vars
  5) .env file (ZINDIAN_COMPETITION or COMPETITION_SLUG)
  6) Auto-detection (when exactly one competitions/*/SKILL_STATE.json exists)
  7) Legacy root fallback
```

**Concurrency & Thread-Safety Rule:** `resolve_competition_paths` MUST NOT mutate process-global state (such as `os.environ["COMPETITION_SLUG"]`). Path resolution functions are pure readers, allowing multiple concurrent competition execution threads within a single Python process without cross-thread context leaking.
...
policy_writer():
    ...
    Fields: allowed_features, blocked_features,
            block_reasons, automl_permitted
    Rule  : automl_permitted is synthesized via strict boolean AND
            between config.get("automl_permitted") and not comp.get("automl_banned").
            Explicit config prohibitions take precedence over unverified monitor defaults.
...
For each target_spec in target_config["targets"]:
    ...
    Write per-target results into shap_multi_target_results.

Backfill Contract (CLI & Report Parity):
    _run_multi_target_shap() MUST backfill the primary target's
    results (top feature, top features list, feature count) into root
    state keys shap_top_feature, shap_top_features, and shap_feature_count.
...
All historical findings, vulnerabilities (VULN-1, VULN-2, VULN-3), and resolved feature gaps (C1–C4, S1–S11, F1–F4) have been moved to the dedicated audit trail file: **[`docs/resolved_gaps.md`](file:///home/adrian/Projects/zindian-orchestrator/docs/resolved_gaps.md)**.
